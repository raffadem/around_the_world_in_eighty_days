...
class AroundTheWorld(object):
    ...
    def __init__(self,
        ...
        """
        self._x_size_default: float = x_size
        self._y_size_default: float = y_size
        """
        _coord_start : (float, float)
            Coordinates of the starting city declared as (longitude, latitude)
        """
        self._coord_start: (int, int) = None  # (lng, lat)
        """
        _city_step : str
        ...
        """
        self._city_step: str = city_start
        self._country_step: str = country_start
        self._coord_step: (int, int) = None  # (lng, lat)
        """
        _lat_max : float
    ...
    def _get_coordinates(self):
        """Get the longitude and the latitude of the starting city.
        """
    ...
    def generate_grid(self):
        ...
        if self._coord_step[1] >= self._coord_start[1]:
            ...
            # As latitude min is used the latitude of the start city minus half of the size of y_size
            self._lat_min = self._coord_start[1] - self.y_size / 2
        else:
            # As latitude max is used the latitude of the start city plus half of the size of y_size
    ...
    def check_grid_city(self) -> pd.DataFrame:
        ...
        # If the rows are less than n_min
        if n_row < self.n_min:
    ...
    def stop_condition(self, grid_city: pd.DataFrame) -> pd.DataFrame:
        ...
        if it true, it returns a dataframe with only the row of the starting city
    ...
    def travel(self, is_intermediate_plot: bool = True, n_intermediate_step: int = 100, is_clear_output: bool = False):
        ...
        if is_intermediate_plot:
            self.plot_world(is_clear_output)
        print(f"Completed the journey departing from {self.city_start} ({self.country_start})",
